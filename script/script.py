...
import math
#因为测试时数量有点大，所以设置了nums_buck参数，它表示top crashes中buckets的数量
        ...
        for j in range(pages):
            ...
            for k in range(len(problems)):
                ...
                if len(info) > 0:
                    ...
                    function=crash.find_all('a')
                    if len(function) > 0:
                        function=crash.find_all('td',title=True)
                        for i in range(len(function)):
                            functions.append(str(function['title']))
